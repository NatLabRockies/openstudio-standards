...
class NECB_HVAC_System_3_Matrix_Test < Minitest::Test
  include NecbHelper

  matrix = NecbHelper.test_case_matrix(
    boiler_fueltype:   ['NaturalGas', 'Electricity', 'FuelOilNo2'],
    heating_coil_type: ['Electric', 'Gas', 'DX'],
  ...
  matrix.each do |case_args|
    ...
    test_name       = "test_necb_hvac_system_3_#{fuel_token}_#{heating_token}_#{baseboard_token}"
    ...
    define_method(test_name) do
      ...
      name = "sys3_Boiler-#{case_args[:boiler_fueltype]}_HeatingCoilType-#{case_args[:heating_coil_type]}_BaseboardType-#{case_args[:baseboard_type]}"
