...
module BTAP
  module Resources
    module Envelope
      ...
      # This method assignes interior surface construction to adiabatic surfaces
      # from model.
      ...
      def self.assign_interior_surface_construction_to_adiabatic_surfaces(model, runner = nil)
        ...
        unless model.building.get.defaultConstructionSet.empty? or model.building.get.defaultConstructionSet.get.defaultInteriorSurfaceConstructions.empty? or model.building.get.defaultConstructionSet.get.defaultInteriorSurfaceConstructions.get.wallConstruction.empty?
          ...
          # Give adiabatic surfaces a construction. Does not matter what. This
          # is a bug in OpenStudio that leave these surfaces unassigned by the
          # default construction set.
          all_adiabatic_surfaces = BTAP::Geometry::Surfaces::filter_by_boundary_condition(
            model.getSurfaces, "Adiabatic")
      ...
      # This module contains methods dealing with the creation and modification
      # of constructions.
      module Constructions # Resources::Envelope::Constructions
        ...
        # This method will create a new construction based on self and a new
        # conductance value. It will check to see if a similar construction has
        # already been created by this method if so it will return the existing
        # construction. If you wish to keep some of the properties, enter the
        ...
        def self.customize_opaque_construction(model, construction, conductance)
          ...
          if conductance.kind_of?(Float)
            ...
            # Determine how low the resistance can be set. Subtract exisiting
            # insulation Values from the total resistance to see how low we
            # can go.
            ...
            if minimum_resistance > (1 / conductance)
              ...
              # Changing the insulation layer will not be enough so either
              # start removing layers or modify them to get to the required
              # conductance.
        ...
        # This removes construction layers if the required conductance for a
        # construction is higher than the maximum conductance that construction
        # can have. Otherwise it modifies existing layers to set their thickness
        # or resistance values (depending on what is in the layer) to achieve
        # the required conductance.
        ...
        def self.adjust_opaque_construction(construction:, req_conductance:)
          ...
          # Extract the thickness, conductivity, resistance of each layer of the
          # construction.  If the material is "No Mass", or "Air gap" set the
          # conductivity (how well the material conducts heat) and conductance
          # (how well the material with a given thickness conducts heat) to
          # the inverse of the resistance.  This is because No Mass and Air gap
          # materials do not have a thickness value.  Also, include which layer
          # index the material has and the material object itself.
          ...
          # Sort the above layers by the conductivity of the layers.  The lowest
          # conductivity layers first followed by layers with progressively
          ...
          # The following loop steps through the array of layers, sorted
          # form highest conductivity to lowest.  It deletes a layer in the
          # construction if the conductance for the layer is not enough to reach
          # the total conductance for the construction that we are trying to
          # reach.  If modifies the thickness or resistance (depending on the
          # layer material type) of a layer if doing so will reach the overall
          # construction conductance target. The total conductance of the
          # construction is rounded because the conductance never seems to be
          # set precisely enough.
          while total_conductance.round(4) < req_conductance
            ...
            # Note that both the construction array and sorted array contain
            # the same elements. However these elements may be in a different
            # order. Thus, the index and const_index may be different. They
            # will both indicate the same layer. However, they may differ
            # because the sorted array is sorted by conductivity while the
            # construction array is ordered with the first layer outside (a
            # given space) and the final layer inside (a given space).
            const_index = sorted_layers[index][:construction_index]

            # Check if modifying the resistance of the currently layer will be
            # enough to reach our total construction conductance goal.  If it
            # will, modify the layer.  If it will not, delete the layer.
            if sorted_layers[index][:resistance_SI] > ((1.0 / total_conductance) - (1.0 / req_conductance))

              # If the current layer is a NoMass or AirGap material its
              # thickness is zero so we set the resistance.
              if sorted_layers[index][:thickness_m] == 0
                ...
                # Find out if the layer is an AirGap or NoMass and set the
                # resistance for the layer with the right command systax.
                ...
                # The the current layer is a regular opaque material it has a
                # thickness so we set that to reach the desired resistance for
                # that layer. Determine the thickness we want to set the layer.
              ...
              # Step the index of the sorted array forward by 1.  We should be
              # able to leave the loop now because the construction should have
              # the conductance we want now.  But you never know.
              index += 1
            else

              # There the layer could not be adjusted to reach the desired
              # conductance for the construction so get rid of the layer. If
              # this is the only layer then we cannot get rid of it so throw an
              # error. This should never happen but you never know.
              ...
              # Delete the layer from the sorted set of layers (so that both
              # the construction array and sorted array continue to contain the
              # same layers).
              sorted_layers.delete_at(index)

              # Go through the sorted array and change the construction indicies
              # so that they continue to point to the correct layers of the
              # construction array. Note that index is not increased. This is
              # because the element we were looking at just got removed so its
              # index will be the same as that of what would have been the next
              # element.
            ...
            # Get the revised conductance for the construction now that it has
            # been modified (by either removing or modifying layers).
            total_conductance = construction.thermalConductance.to_f

            # Check if we have anything left to modify. If yes, then keep going.
            # If not, then if we have done enough we can stop and return the
            # revised construction, otherwise throw an error.
        ...
        def self.customize_fenestration_construction(
          ...
          # Set value of fictious glazing based on the fenestrations front and
          # back if available.
          ...
          # Create fictious glazing. Assume a thickness of 0.10m.
          thickness = 0.10
          ...
          # Add the glazing and any shading materials to the array and create
      ...
      module ConstructionSets #Resources::Envelope::ConstructionSets
        ...
        def self.customize_default_surface_construction_set!(
          model:,
          name:,
        ...
        # This creates a new construction set of wall, floor and roof/ceiling
        # objects.
        # @author phylroy.lopez@nrcan.gc.ca
        # @param model [OpenStudio::Model::Model]
        # @param name [String] = nil
        ...
        # This method creates a subsurface construction set (windows, doors,
        # skylights, etc)
        # @author phylroy.lopez@nrcan.gc.ca
        # @param model [OpenStudio::Model::Model]
