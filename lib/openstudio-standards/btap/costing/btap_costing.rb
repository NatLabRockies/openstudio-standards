  ...
  class Costing
    ...
    def initialize(costs_csv: nil, factors_csv: nil, attributes:)
      ...
      if (not costs_csv.nil?) and File.exist?(costs_csv)
        Paths.costs_path = costs_csv
      end

      # If the path for custom factors is defined, use custom factors.
      if (not factors_csv.nil?) and File.exist?(factors_csv)
        Paths.costs_local_factors_path = factors_csv
      end
    end

    def validate_database()
      @costing_database.validate_ahu_items_and_quantities
    end
    ...
    def cost_audit_all(model:,
      ...
      # Create an array to collect costed item information. First element is the
      # costing location.
      @cost_items = {
      ...
      # Create nested hashes for each category of costing.
      ...
      # Check to see if standards building type and the number of stories has
      # been defined. The former may be omitted in the future.
      ...
      @costing_report["totals"] = {
        ...
        'grand_total'         => (
          envCost + thermalBridgingCost + lgtCost + boilerCost + chillerCost + coolingTowerCost + shwCost + ventCost +
          zonalSystemCost + pvGroundCost).round(0)
      }
    ...
    def read_mech_sizing()
      return JSON.parse(File.read(Paths::MECH_SIZING_PATH))
    end
    ...
    # costing.
    # @param id [String]  The costing database id for the item being costed.
    # @param quantity [Float] The total amount of the item being costed in
    #   whatever units the item is costed in. This should include all multiplier
    #   used to determine this cost (e.g. such as thermal_zone multipliers).
    #   As an example, if 32 ft. of wire were required for a piece of equipment
    #   used in a thermal zone with a multiplier of 10, the quantity would be
    #   3.2 (32 ft. * 10 / 100 since wire is costed per 100 ft.).
    #
    # @param material_mult [Float] The multiplier used to estimate the cost
    #   of an item from the base cost. For example, high efficiency SHW tanks
    #   are estimated to cost 30% higher than regular SHW tanks so the cost of
    #   these tanks are calculated by the cost * 1.3. Thus, the material_mult
    #   for high efficiency tanks would be 1.3. This is defauted to 1.0 if it is
    #   not provided.
    #
    # @param labour_mult [Float] Similar to material_mult only applied to labour
    #   costs. The labour_mult can be different than the material_mult. This is
    #   defaulted to 1.0 if it is not provided. equipment_mult: (float) Similar
    #   to material_mult and labour_mult only for equipment. This will always be
    #   1.0 until equipment costs are supported.
    #
    # @param tags [Array[String]] This is an array which links the costed
    #   item to a component of the model that is being costed.  For example,
    #   a material_id related to a boiler pump might have tags like ["boiler",
    #   "pump"].
    ...
    # This method takes the list of costed items in the building generated with
    # the help of the above add_costed_item method and finds the costs for the
    # list of items.
    # @param btap_items [Array[Hash]] This array contains all the items that
    #   must be costed. The first element of the array is:
    #     {
    ...
    #       quantity: [Float]  Amount of costed item (should include all
    #         multipliers except localization factors, material_mult,
    #         labour_mult, equipment_mult).
    #
    #       material_mult: [Float] Material multiplier from cost spreadsheet used
    #         mainly for higher performance equipment (for example, regular and
    #         high performance boilers share the same id but high performance
    #         boilers have a material_mult of around 1.3-that is they are
    #         estimated to be 1.3 times as expensive as regular boilers).
    #
    #       labour_mult: [Float] Same idea as material_mult only for labour
    #         (often this will be 1.0 even if material_mult is something else).
    #
    #       equipment_mult: [Float] Same idea as labour_mult only for equipment.
    #         It will always be 1.0 until equipment costs are implemented in
    #         costing.
    #
    #       tags: [Array[String]] An array of strings used to define what
    #         part of the building is being costed (e.g. an component for
    #         a ccashp might have these tags: "Ventilation", "CCASHP",
    #         "ccashp_condensor")
    #     }
    #
    # @param custom_costing [Array[Hash]] A custom costing database if you do
    #   not want to use the default one. This must have the same format as that
    #   found by @costing_database['costs']
    #
    # @param custCity [String] A custom cost localization city if you do not
    #   want to use the one in the first item in the btap_itmes hash.
    #
    # @param custProvince [String] A custom cost localization province if you do
    #   not want to use the one in the first item in the btap_items hash.
    #
    # @return [Hash] A hash containing these summary costs:
    #   {
    #     envelope: [Float] Building envelope costs (to 2 decimal places).
    #     lighting: [Float] Ligting costs (to 2 decimal places).
    #     heating_and_cooling: [Float] Heating and cooling costs (not related to
    #       ventilation) (to 2 decimal places).
    #
    #     shw:         [Float] Service hot water costs (to 2 decimal places).
    #     ventilation: [Float] Ventilation (including ventilation air heating
    #       and cooling) costs (to 2 decimal places).
    #
    ...
    def cost_list_items(btap_items:, custom_costing: nil, custCity: nil, custProvince: nil)
      ...
      # Check if costing is for a custom city and province. If not use the city
      # and province found in the first entry of the array of costed items.
      ...
      btap_items['Items'].each do |costing_item|
        ...
        if costing_data.nil?
          ...
          # This is a stub for some work that needs to be done to account for
          # equipment costing. For now this is zeroed out. A similar test is
          # done on reading the data from the database and collected in the
          # error file when the costing database is generated.
          raise "Error: costing information for material id #{costing_item['id']} is nil.  Please check costing data."
        end

        costing_data['baseCosts']['equipmentOpCost'].nil? ? equip_base_cost = 0.0 : equip_base_cost =
          costing_data['baseCosts']['equipmentOpCost'].to_f
        costing_data['baseCosts']['materialOpCost'].nil? ? mat_base_cost    = 0.0 : mat_base_cost   =
          costing_data['baseCosts']['materialOpCost'].to_f
        costing_data['baseCosts']['laborOpCost'].nil? ? lab_base_cost       = 0.0 : lab_base_cost   =
          costing_data['baseCosts']['laborOpCost'].to_f

        # The costs from the costing database are US national average costs
        # (for placeholder costs) or whatever is in the 'province_state' and
        # 'city' fields (for custom costs). These costs need to be adjusted
        # to reflect the costs expected in the location of interest. The
        # 'get_regional_cost_factors' method finds the appropriate cost
        # adjustment factors.
        ...
        # Get any associated material or labour multiplier for the equipment
        # present in the 'materials_hvac' sheet in the costing spreadsheet.
        ...
        costing_item['equipment_mult'].to_f == 0 || costing_item['equipment_mult'].nil? ? eq_quant = 1.0 :
          eq_quant = costing_item['equipment_mult'].to_f
        ...
        heatCoolCost += item_cost unless (costing_item['tags'].select { |data|
          data.to_s.upcase == "HEATING_COOLING" }).empty?
        shwCost      += item_cost unless (costing_item['tags'].select { |data| data.to_s.upcase == "SHW" }).empty?
        ventCost     += item_cost unless (costing_item['tags'].select { |data|
          data.to_s.upcase == "VENTILATION" }).empty?
