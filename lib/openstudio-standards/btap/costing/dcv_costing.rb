module BTAP
  class Costing
    def cost_audit_dcv(model:, prototype_creator:)
      ...
      # These inputs are required for the calculation of the length of conduit
      # for each AirLoopHVAC.
      util_dist, ht_roof, nominal_flr2flr_height_ft, horizontal_dist, standards_number_of_stories, mechRmInBsmt =
        getGeometryData(model, prototype_creator)
      ...
      model.getAirLoopHVACs.sort.each do |air_loop|
        ...
        # Step A: Calculate number of thermal zones and floors served by each
        # AirLoopHVAC (air_loop).
        building_story_list = []
        ...
        air_loop.supplyComponents.sort.each do |supply_component|
          ...
          if !hvac_component.empty?
            ...
            # Check if "Demand Controlled Ventilation" is "Yes" in
            # Controller:MechanicalVentilation depending on dcv_type.
            controller_mechanical_ventilation_demand_controlled_ventilation_status =
              controller_mechanical_ventilation.demandControlledVentilation

            controller_mechanical_ventilation_system_outdoor_air_method =
              controller_mechanical_ventilation.systemOutdoorAirMethod()

            if controller_mechanical_ventilation_demand_controlled_ventilation_status == true &&
               (controller_mechanical_ventilation_system_outdoor_air_method == 'ZoneSum' ||
               controller_mechanical_ventilation_system_outdoor_air_method  == 'IndoorAirQualityProcedure')

              # Step B: Calculate costs of the installation of a single junction
              # box for each floor within that AirLoopHVAC (air_loop) to
              # accumulate the wiring.
              quantity_ahu_floors_junction_box = 1.0 * number_floors_served_by_air_loop
              search_ahu_floors_junction_box   = { row_id_1: 'Ea', row_id_2: 14 }
              ...
              # Step C: Calculate costs of the installation of a single conduit
              # that runs the entire height of the building for each AirLoopHVAC
              # to accumulate the wiring.
              quantity_ahu_vertical_wiring = 1.0/100.0 * standards_number_of_stories * nominal_flr2flr_height_ft
              search_ahu_vertical_wiring   = { row_id_1: 'CLF', row_id_2: 10 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              search_ahu_vertical_conduit   = { row_id_1: 'LF', row_id_2: 13 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              # Step D: Calculate the roof conduit and wiring for each
              # AirLoopHVAC.
              quantity_ahu_roof_wiring = 20.0/100.0
              search_ahu_roof_wiring   = { row_id_1: 'CLF', row_id_2: 10 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              search_ahu_roof_conduit   = { row_id_1: 'LF', row_id_2: 13 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              search_ahu_roof_junction_box   = { row_id_1: 'Ea', row_id_2: 14 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              search_ahu_contorller   = { row_id_1: 'Ea', row_id_2: 400 }
              sheet_name = 'materials_lighting'
              column_1   = 'unit'
              column_2   = 'lighting_type_id'
              ...
              if controller_mechanical_ventilation_system_outdoor_air_method == 'ZoneSum'
                ...
                # Calculate occupancy sensor-related costs of each thermal zone
                # served by each AirLoopHVAC (air_loop)
                quantity_tz_occupancy_sensor = 1.0 * number_of_thermal_zones_served_by_air_loop.to_f
                search_tz_occupancy_sensor   = { row_id_1: 'Ea', row_id_2: 404 }
                sheet_name = 'materials_lighting'
                column_1   = 'unit'
                column_2   = 'lighting_type_id'
                ...
                search_tz_occupancy_sensor_wiring   = { row_id_1: 'CLF', row_id_2: 10 }
                sheet_name = 'materials_lighting'
                column_1   = 'unit'
                column_2   = 'lighting_type_id'
                ...
                search_tz_occupancy_sensor_pvc_conduit   = { row_id_1: 'LF', row_id_2: 17 }
                sheet_name = 'materials_lighting'
                column_1   = 'unit'
                column_2   = 'lighting_type_id'
                cost_tz_occupancy_sensor_pvc_conduit = assembly_cost(
                  cost_info:  search_tz_occupancy_sensor_pvc_conduit,
                  sheet_name: sheet_name,
                  column_1:   column_1,
                  column_2:   column_2,
                ...
                dcv_cost_zone_occupancy =
                  cost_tz_occupancy_sensor + cost_tz_occupancy_sensor_wiring + cost_tz_occupancy_sensor_pvc_conduit
                dcv_cost_total +=
                  dcv_cost_zone_occupancy + dcv_cost_box + dcv_cost_vertical_conduit + dcv_cost_roof + dcv_cost_control
                total_cost_for_air_loop =
                  dcv_cost_zone_occupancy + dcv_cost_box + dcv_cost_vertical_conduit + dcv_cost_roof + dcv_cost_control

              elsif controller_mechanical_ventilation_system_outdoor_air_method == 'IndoorAirQualityProcedure'
                # Step F: Calculate total Cost for each AirLoopHVAC
                # Calculate CO2 sensor-related costs of each thermal zone served
                # by each AirLoopHVAC (air_loop).
                quantity_tz_co2_sensor = 1.0 * number_of_thermal_zones_served_by_air_loop.to_f
                search_tz_co2_sensor   = { row_id_1: nil, row_id_2: 1316 }
                ...
                search_tz_co2_sensor_wiring   = { row_id_1: 'CLF', row_id_2: 10 }
                sheet_name = 'materials_lighting'
                column_1   = 'unit'
                column_2   = 'lighting_type_id'
                ...
                search_tz_co2_sensor_pvc_conduit   = { row_id_1: 'LF', row_id_2: 17 }
                sheet_name = 'materials_lighting'
                column_1   = 'unit'
                column_2   = 'lighting_type_id'
                ...
                dcv_cost_total   +=
                  dcv_cost_zone_co2 + dcv_cost_box + dcv_cost_vertical_conduit + dcv_cost_roof + dcv_cost_control
                total_cost_for_air_loop =
                  dcv_cost_zone_co2 + dcv_cost_box + dcv_cost_vertical_conduit + dcv_cost_roof + dcv_cost_control
              end
