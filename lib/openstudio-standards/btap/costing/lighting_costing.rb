module BTAP
  class Costing
    def cost_audit_lighting(model, prototype_creator)
      ...
      # Calculate daylighting sensor control and lighting costing.
      ...
      model.getThermalZonesSorted.each do |zone|
        ...
        zone.getSpacesSorted.each do |space|
          ...
          # Get the SpaceType of the space. If it is not defined it, it will
          # skip the spacetype.
          ...
          if space_has_no_light == false
            space.spaceType.get.lights.sort.each do |light|
              ...
              space_lights_definition_name = space_lights_definition.name
               ...
               # NOTE: NECB2020's lights default is LED.
              if space_lights_definition_name.to_s.include?('LED lighting') or template_type == 'NECB2020'
                light_type = 'LED'
              else
                light_type = 'CFL'
              end
          ...
          # Get standard lighting sets based on collected information
          # (spacetype, no of stories, etc.).
          if space_has_no_light == false
          ...
          # Determine average space height using space volume and floor area
          # (convert to feet).
          ceilHgt, flrArea = 0
          ...
          # Costs are 0 for 'Nil' because no fixture type due to either zero
          # floor area, zero ceiling height or a 'Nil' setting for fixture type
          # in lighting_sets sheet ("- undefined -" space).
          if fixtureType != 'Nil'
            ...
            if lighting_type["id_layers"].empty?
              ...
              lighting_layers.each do |layer_id, layer_mult|
                ...
                # NOTE: The column in `lighting.csv` labelled
                # "lighting_type_id" is mislabelled and should really be
                # "lighting_type_layer_id" but left it as-is (below).
                ...
                if lighting_material.nil?
                  ...
                  if costing_data.nil?
                    ...
                    material_cost =
                      costing_data['baseCosts']['materialOpCost'].to_f * layer_mult.to_f * flrArea * zone.multiplier

                    labour_cost =
                      costing_data['baseCosts']['laborOpCost'].to_f * layer_mult.to_f * flrArea * zone.multiplier

                    layer_MaterialCost += material_cost
                    layer_LabourCost   += labour_cost

                    regional_material, regional_installation =
                      get_regional_cost_factors(@costing_report["province_state"],
                                                @costing_report["city"],
                                                lighting_material)

                    total_with_region =
                      layer_MaterialCost * regional_material / 100.0 + layer_LabourCost * regional_installation / 100.0
          ...
          unless lighting_fixture_report.nil?
            lighting_fixture_report['floor_area_ft2'] =
              (lighting_fixture_report['floor_area_ft2'] + (flrArea * space.multiplier)).round(1)

            lighting_fixture_report['cost'] = (lighting_fixture_report['cost'] + total_with_region).round(2)
            lighting_fixture_report['cost_per_ft2'] =
              (lighting_fixture_report['cost'] / lighting_fixture_report['floor_area_ft2']).round(2)
