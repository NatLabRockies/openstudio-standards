module BTAP
  class Data
    ...
    class << self
      ...
      def look_up_csv_data(csv_fname, search_criteria)
        ...
        CSV.open(csv_fname, 'r', **options) do |csv|
          ...
          # Since CSV includes Enumerable we can use 'find_all' which will
          # return all the elements of the Enumerble for which the block returns
          # true
      ...
      def get_total_nominal_capacity(model)
        ...
        model.getSpaces.sort.each do |space|
          unless space.thermalZone.empty?
            ...
            area = self.validate_optional(area, model, -1)
            ...
            multiplier = self.validate_optional(multiplier, model, -1)
            ...
            area_per_person = self.validate_optional(area_per_person, model, -1)
    ...
    def utility(model:, utility_pricing_year: 2020)
      ...
      #    costing_rownames = self.class.validate_optional(costing_rownames, model, "N/A")
    ...
    def air_loops_table(model, cost_result)
      ...
      model.getAirLoopHVACs.sort.each do |air_loop|
        ...
        air_loop_info['area_outdoor_air_rate_m_cu_per_s_m_sq'] = self.class.validate_optional(model.sqlFile.get.execAndReturnFirstDouble(sql_command), model, -1.0)
        ...
        air_loop.thermalZones.sort.each do |zone|
          ...
          vbz = self.class.validate_optional(model.sqlFile.get.execAndReturnFirstDouble(sql_command), model, 0)
        ...
        unless air_loop.supplyFan.empty?
          ...
          max_air_flow_info = self.class.validate_optional(max_air_flow_info, model, 'N/A')
    ...
    def eplusout_err_table(model)
      ...
      warnings = self.class.validate_optional(warnings, model, 'N/A')
      unless warnings == 'N/A'
    ...
    def service_water_heating_data
      ...
      service_water_heating['shw_total_nominal_occupancy'] = self.class.get_total_nominal_capacity(@model)
      ...
      service_water_heating['shw_electricity_per_year'] = self.class.validate_optional(service_water_heating['shw_electricity_per_year'], @model, -1)
      ...
      service_water_heating['shw_natural_gas_per_year'] = self.class.validate_optional(service_water_heating['shw_natural_gas_per_year'], @model, -1)
      ...
      service_water_heating['shw_additional_fuel_per_year'] = self.class.validate_optional(service_water_heating['shw_additional_fuel_per_year'], @model, -1)
      ...
      service_water_heating['shw_water_m_cu_per_year'] = self.class.validate_optional(service_water_heating['shw_water_m_cu_per_year'], @model, -1)
