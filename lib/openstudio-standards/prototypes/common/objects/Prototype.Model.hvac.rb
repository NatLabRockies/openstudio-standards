    ...
    @system_to_space_map.each do |system|
      ...
      when 'VAV'
        ...
        hot_water_loop = if model.getPlantLoopByName('Hot Water Loop').is_initialized
                           ...
                           OpenstudioStandards::HVAC.model_add_hw_loop(model,
                                             'NaturalGas',
      ...
      when 'CAV'
        ...
        hot_water_loop = if model.getPlantLoopByName('Hot Water Loop').is_initialized
                           ...
                           OpenstudioStandards::HVAC.model_add_hw_loop(model, 'NaturalGas')
                         end
        ...
        # Add the CAV
        OpenstudioStandards::HVAC.model_add_cav(model,
                      thermal_zones,
                      system_name: system['name'],
                      hot_water_loop: hot_water_loop,
                      chilled_water_loop: chilled_water_loop,
      ...
      when 'PSZ-AC'
        ...
        if system['heating_type'] == 'Water To Air Heat Pump'
          ...
          heat_pump_loop = OpenstudioStandards::HVAC.model_get_or_add_heat_pump_loop(model, 'NaturalGas', 'Electricity', heat_pump_loop_cooling_type: hp_loop_cooling_type)
        end
      ...
      when 'DC' # Data Center in Large Office building
        # Retrieve the existing hot water loop or add a new one if necessary.
        hot_water_loop = OpenstudioStandards::HVAC.model_get_or_add_hot_water_loop(model, 'NaturalGas')
        ...
        heat_pump_loop = OpenstudioStandards::HVAC.model_get_or_add_heat_pump_loop(model, 'NaturalGas', 'Electricity',
                                                         heat_pump_loop_cooling_type: heat_pump_loop_cooling_type)
        OpenstudioStandards::HVAC.model_add_data_center_hvac(model,
                                   thermal_zones,
                                   hot_water_loop,
                                   heat_pump_loop,
      ...
      when 'CRAC' # Small Data Center
        OpenstudioStandards::HVAC.model_add_crac(model,
                       thermal_zones,
                       climate_zone,
                       system_name: system['name'],
                       hvac_op_sch: hvac_op_sch,
      ...
      when 'CRAH' # Large Data Center (standalone)
        ...
        OpenstudioStandards::HVAC.model_add_crah(model,
                       thermal_zones,
                       system_name: system['name'],
                       chilled_water_loop: chilled_water_loop,
                       hvac_op_sch: hvac_op_sch,
      ...
      when 'SAC'
        OpenstudioStandards::HVAC.model_add_split_ac(model,
                           thermal_zones,
                           cooling_type: system['cooling_type'],
                           heating_type: system['heating_type'],
                           supplemental_heating_type: system['supplemental_heating_type'],
      ...
      when 'WSHP'
        condenser_loop = case system['heating_type']
                         when 'Gas'
                           OpenstudioStandards::HVAC.model_get_or_add_heat_pump_loop(model,
                                                           system['heating_type'],
                                                           system['cooling_type'],
                                                           heat_pump_loop_cooling_type: 'CoolingTowerTwoSpeed')
                         else
                           OpenstudioStandards::HVAC.model_get_or_add_ambient_water_loop(model)
                         end
      ...
      when 'Fan Coil'
        ...
        when 'Gas', 'DistrictHeating', 'DistrictHeatingWater', 'DistrictHeatingSteam', 'Electricity'
          hot_water_loop = OpenstudioStandards::HVAC.model_get_or_add_hot_water_loop(model, system['heating_type'])
        when nil
          hot_water_loop = nil
        end
        case system['cooling_type']
        when 'Electricity', 'DistrictCooling'
          chilled_water_loop = OpenstudioStandards::HVAC.model_get_or_add_chilled_water_loop(model, system['cooling_type'], chilled_water_loop_cooling_type: 'AirCooled')
        when nil
          chilled_water_loop = nil
        end
        OpenstudioStandards::HVAC.model_add_four_pipe_fan_coil(model,
