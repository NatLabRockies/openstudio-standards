module OpenstudioStandards
  ...
  module SpaceType
    ...
    # Path to the validated mapping of level-1 typical space types to standards space types,
    # generated by utilities/schedule_editor/generate_ashrae_space_type_map.rb
    LEVEL_1_STANDARDS_SPACE_TYPE_MAP_PATH = File.join(__dir__, 'data', 'level_1_standards_space_type_map.json')

    # Resolve the standards building type and space type pair equivalent to a level-1 typical
    # space type for a template. ComStock templates resolve through their base template's rows,
    # and DEER vintages after the newest mapped vintage (DEER 2025 on) resolve to the newest
    # mapped DEER row, whose space type data they share.
    #
    # @param level_1_space_type [String] level-1 typical space type name, e.g. 'office'
    # @param template [String] OpenStudio Standards template, e.g. '90.1-2013'
    # @return [Hash, nil] mapping row with 'building_type' and 'space_type' keys, or nil when
    #   the space type has no equivalent for the template
    def self.level_1_standards_space_type_mapping(level_1_space_type, template)
      @level_1_standards_space_type_map ||= JSON.parse(File.read(LEVEL_1_STANDARDS_SPACE_TYPE_MAP_PATH))
      rows = @level_1_standards_space_type_map[level_1_space_type] || []
      map_template = template.to_s.sub(/^ComStock /, '')
      row = rows.find { |r| r['template'] == map_template }
      # the map rows are ordered newest DEER vintage first
      if row.nil? && map_template.start_with?('DEER')
        row = rows.find { |r| r['template'].start_with?('DEER') }
      end
      row
    end
